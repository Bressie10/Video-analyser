...
class MetaLibraryDatabaseTests(unittest.TestCase):
    ...
    def test_library_exposes_internal_account_identity_and_type(self):
        self.drain()
        with repo.database() as db:
            db.execute("UPDATE meta_accounts SET label='Same business'")
        response = self.api.get("/api/meta/library?limit=100")
        self.assertEqual(response.status_code, 200)
        rows = response.json()["items"]
        accounts = {}
        for item in rows:
            UUID(item["account_id"])
            self.assertEqual(item["account_label"], "Same business")
            accounts[item["account_id"]] = item["account_platform"]
            if item["content_type"] == "ad":
                self.assertEqual(item["account_platform"], "meta_ads")
                for asset in item["assets"]:
                    UUID(asset["account_id"])
            elif item["label"] == "Video ad":
                self.assertEqual(item["platform"], "facebook")
                self.assertEqual(item["account_platform"], "meta_ads")
        self.assertEqual(set(accounts.values()), {"facebook", "instagram", "meta_ads"})
        self.assertEqual(len(accounts), 3)
        for forbidden in ("external_id", "page_external_id", "token_ciphertext"):
            self.assertNotIn(forbidden, response.text)
