...
BEGIN
    ...
    VALUES (test_video_id, 'instagram', 120, 12, 3, 2);
    ...
    IF NOT EXISTS (
        SELECT 1 FROM video_performance
        WHERE video_id = test_video_id AND view_count = 120
          AND source = 'instagram' AND like_count = 12 AND comment_count = 3 AND share_count = 2
    ) THEN
        RAISE EXCEPTION 'Video performance data did not round-trip';
    END IF;
END $$;
