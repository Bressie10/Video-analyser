"""TEST_DATABASE_URL enables an isolated-schema Reel API/repository round trip."""
...
class InstagramDatabaseTests(unittest.TestCase):
    def test_reel_fetch_attach_refresh_and_readback_for_correct_video(self):
        schema = "instagram_test_" + uuid4().hex
            ...
            try:
                ...
                with psycopg.connect(url, autocommit=True) as db:
                    ...
                    with patch.dict(os.environ, {**CONFIG, "DATABASE_URL": url}):
                        ...
                        with graph_transport(metrics=insights({"views": 0, "likes": None})):
                            response = attach(api, target)
                        self.assertEqual(response.status_code, 200)
                        refreshed = {
                        ...
                        self.assertEqual(rows[0][:5], ("instagram", 0, None, None, None))
                        self.assertGreater(rows[0][5], first_time)
                        refreshed_time = rows[0][5]

                        with graph_transport(metrics={"data": []}):
                            self.assertEqual(attach(api, target).status_code, 422)
                        self.assertEqual(get_analysis(target)["performance_metrics"], refreshed)
